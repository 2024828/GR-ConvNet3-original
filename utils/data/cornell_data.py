...
class CornellDataset(GraspDatasetBase):
    ...
    def _get_crop_attrs(self, idx):
        ...
        gtbbs = grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx])
    # 直接绕过GraspRectangles的center属性，直接获取第一个抓取框的中心
        if not gtbbs.grs:  # 确保有抓取框数据
            raise ValueError(f"文件 {self.grasp_files[idx]} 中没有有效的抓取框数据")
    # 直接访问第一个GraspRectangle的center属性
        center = gtbbs.grs[0].center
    # 后续后续裁剪逻辑不变
        left = max(0, min(center[1] - self.output_size // 2, 640 - self.output_size))
        top = max(0, min(center[0] - self.output_size // 2, 480 - self.output_size))
        return center, left, top
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        ...
        if isinstance(zoom, torch.Tensor):
            zoom = zoom.item()  # 从张量提取标量值
        
        # 再次校验 zoom 是标量（确保转换成功）
        if not np.isscalar(zoom):
            raise ValueError(f"缩放因子 zoom 必须是标量！当前类型: {type(zoom)}, 值: {zoom}")
