...
class DepthImage(Image):
    ...
    def inpaint(self, missing_value=0):
        ...
        print(f"初始深度图数据类型: {self.img.dtype}")
        self.img = np.asarray(self.img, dtype=np.float32)
        print(f"强制转换后初始类型: {self.img.dtype}")
        ...
        if scale == 0:  # 避免除以零（极端情况：全黑图像）
            scale = 1.0
        self.img = self.img / scale 
        
        print(f"深度图形状（高,宽,通道数）: {self.img.shape}")  # 比如单通道应该是 (480,640)，3通道是 (480,640,3)
        print(f"深度图数据类型: {self.img.dtype}")              # 比如 uint16（Cornell深度图常见类型）、float64 等
