...
class GraspRectangles:
    ...
    def __getattr__(self, attr):
        ...
        # 原逻辑：动态查找GraspRectangle类中的可调用方法（仅对方法生效）
        if hasattr(GraspRectangle, attr) and callable(getattr(GraspRectangle, attr)):
            return lambda *args, **kwargs: list(map(lambda gr: getattr(gr, attr)(*args, **kwargs), self.grs))
        else:
    ...
    def center(self):
        ...
        """
        if not self.grs:  # 避免空列表报错
            raise ValueError("当前没有加载任何抓取框数据")
    # 返回第一个抓取框的 center 属性（GraspRectangle 类已正确实现单个框的中心）
        return self.grs[0].center
...
class GraspRectangle:
    ...
    def rotate(self, angle, center):
        ...
        """
        c = np.array(center, dtype=np.float32)  # 统一数据类型，避免维度干扰
        c = c.squeeze()  # 移除所有大小为1的维度（如 (2,1)→(2,)，(1,2)→(2,)）
        if c.ndim != 1 or len(c) != 2:  # 防止无效的 center 格式
            raise ValueError(f"旋转中心格式错误！需为 [y,x]，当前形状: {c.shape}")
        c = c.reshape(1, 2) 
        # R = np.array(
        #     [
        ...
        R = np.array([
            [np.cos(-angle), np.sin(-angle)],
            [-np.sin(-angle), np.cos(-angle)]  # 简化原代码的 -1*sin，逻辑一致
        ], dtype=np.float32)
        R = R.squeeze()
        if R.shape != (2, 2):
            raise ValueError(f"旋转矩阵维度异常！预期 (2,2)，当前: {R.shape}")
        points_diff_T = (self.points - c).T  # 形状 (2,4)
        rotated_T = np.dot(R, points_diff_T)  # 形状 (2,4)
        rotated_points = rotated_T.T + c  # 形状 (4,2)
        self.points = rotated_points.astype(int)
    ...
    def zoom(self, factor, center):
        ...
        # self.points = ((np.dot(T, (self.points - c).T)).T + c).astype(int)
                # 1. 校验 factor 是标量（避免因 factor 是数组导致 T 矩阵形状不均匀）
        if not np.isscalar(factor):
            raise ValueError(f"缩放因子 factor 必须是标量！当前类型: {type(factor)}, 值: {factor}")
        # 处理 factor=0 的极端情况（避免除以零）
        if factor == 0:
            raise ValueError("缩放因子 factor 不能为 0")
        scale = 1.0 / factor  # 计算缩放系数

        # 2. 修正 center 维度：确保是 (1,2)，与 self.points (4,2) 广播兼容
        c = np.array(center, dtype=np.float32)
        c = c.squeeze()  # 移除多余维度（如 (2,1)→(2,)，(1,2)→(2,)）
        if c.ndim != 1 or len(c) != 2:
            raise ValueError(f"缩放中心 center 格式错误！需为 [y,x]，当前形状: {c.shape}")
        c = c.reshape(1, 2)  # 转为 (1,2)，与 self.points (4,2) 广播匹配

        # 3. 构建缩放矩阵 T：确保是 (2,2) 均匀形状的2D数组
        T = np.array([
            [scale, 0.0],    # 第一行：x轴缩放系数，无偏移
            [0.0, scale]     # 第二行：y轴缩放系数，无偏移
        ], dtype=np.float32)  # 明确 dtype，避免类型不一致导致形状异常
        # 校验 T 矩阵形状（确保均匀）
        if T.shape != (2, 2):
            raise ValueError(f"缩放矩阵 T 形状异常！预期 (2,2)，当前: {T.shape}")

        # 4. 计算缩放后坐标（维度完全对齐，无不均匀问题）
        # self.points - c: (4,2) - (1,2) → (4,2)（广播正常，形状均匀）
        # .T: 转置为 (2,4)（与 T (2,2) 列数匹配）
        # np.dot(T, ...): (2,2) × (2,4) → (2,4)（矩阵乘法无形状问题）
        # .T: 转置回 (4,2)，与原 self.points 形状一致
        points_diff = self.points - c  # 形状 (4,2)，均匀
        points_diff_T = points_diff.T  # 形状 (2,4)，均匀
        zoomed_T = np.dot(T, points_diff_T)  # 形状 (2,4)，均匀
        zoomed_points = zoomed_T.T + c  # 形状 (4,2)，均匀

        # 5. 类型转换（保持与原代码一致的 int 类型）
        self.points = zoomed_points.astype(int)
